...
exports.deleteReviewById = async (req, res) => {
    try {
        ...
        let user_review_id = req.params.id;
        if (!user_review_id) {
            logs("UserReview", "create", "Info", "Not Found for Delete a user review with id = " + user_review_id);
            // return a response to client
            res.status(404).json({
            ...
            // return the response to client
            if (!result) {
                logs("UserReview", "deleteById", "Error", "Error -> Can not delete a user review with id = " + req.params.id);
                res.status(500).json({
            ...
            res.status(200).json({
                message: "delete successfully a user review with id = " + user_review_id
            });
        }
    } catch (error) {
        logs("UserReview", "deleteById", "Info", "Error -> Can not delete a user review with id = " + req.params.id);
        res.status(500).json({
            message: "Error -> Can not delete a user review with id = " + req.params.id,
            error: errorResponse(error.message)
        });
